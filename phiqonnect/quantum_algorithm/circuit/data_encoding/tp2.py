...
class TP2(PQC):
	...
	def _build(self) -> None:
		...
		first_len: int = len(self._first_circuit)
		latter_len: int = len(self._latter_circuit)
		
		params: ParameterVector = ParameterVector('x', self._feature_dimension)
		...
		for i in range(self._num_qubits):
			for j in range(first_len):
				gate: Any = getattr(qiskit.circuit.library, self._first_circuit[j] + "Gate")			  
		...
		for i in range(self._num_qubits):
			for j in range(latter_len):
				gate: Any = getattr(qiskit.circuit.library, self._latter_circuit[j] + "Gate")
				qc.append(gate(params[i]), [i])
		layer.compose(qc, inplace=True)
