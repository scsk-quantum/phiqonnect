...
from .classical_svc import ClassicalSVC
from ....utils.calcuration.eval_classification_result import multi_class_eval_score

class MulticlassClassicalSVC():
	"""
	MulticlassClassicalSVC(現代のサポートベクター多クラス分類)クラス
	...
	def __init__(self, kernel_func, class_num, method, verbose=False) -> None:
		...
		if self.method == "ovr":
			self.classifiers = [ClassicalSVC(kernel_func, verbose, True) for _ in range(class_num)]
		elif self.method == "ovo":
			self.combinations = list(itertools.combinations(list(range(class_num)), 2))
			self.classifiers = [ClassicalSVC(kernel_func, verbose) for _ in range(len(self.combinations))]
		else:
